<x-app-layout>
    ...
    <div class="py-4">
        <div class="max-w-7xl mx-auto px-4">
            <div class="mb-4 rounded-xl overflow-hidden bg-gradient-to-r from-indigo-700 via-blue-700 to-cyan-600 text-white shadow-lg ring-1 ring-indigo-500/20">
                <div class="p-6 flex items-center justify-between">
                    <div class="flex items-center gap-4">
                        <x-application-logo class="h-12 w-auto rounded-full ring-2 ring-white/40" />
                        <div>
                            <div class="text-xs opacity-80">MCRO SMS Notification</div>
                            <div class="text-2xl font-semibold">Operational Dashboard</div>
                            <div class="text-xs opacity-80">Real-time status and upcoming schedules</div>
                        </div>
                    </div>
                    <div class="grid grid-cols-3 gap-6">
                        <div class="text-center bg-white/10 rounded-md px-4 py-2 ring-1 ring-white/20">
                            <div class="flex items-center justify-center gap-2 text-xs opacity-90">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div class="bg-white border rounded-md">
                    <div class="px-3 py-2 border-b text-sm font-medium text-gray-700 flex items-center justify-between">
                        ...
                        <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs bg-red-100 text-red-700">{{ $countOverdue ?? 0 }}</span>
                    </div>
                    <div class="p-3 space-y-2 max-h-96 overflow-y-auto">
                        @forelse(($schedOverdue ?? []) as $it)
                            @php $due = $it['due']; @endphp
                            <div class="border rounded-md p-2 bg-red-50 shadow-sm border-l-4 border-red-400">
                ...
                <div class="bg-white border rounded-md">
                    <div class="px-3 py-2 border-b text-sm font-medium text-gray-700 flex items-center justify-between">
                        ...
                        <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs bg-yellow-100 text-yellow-700">{{ $countToday ?? 0 }}</span>
                    </div>
                    <div class="p-3 space-y-2 max-h-96 overflow-y-auto">
                        @forelse(($schedToday ?? []) as $it)
                            @php $due = $it['due']; @endphp
                            <div class="border rounded-md p-2 bg-yellow-50 shadow-sm border-l-4 border-yellow-400">
                                <div class="flex justify-between">
                                    <div class="text-sm font-semibold text-gray-900">{{ $due->format('Y-m-d') }}</div>
                ...
                <div class="bg-white border rounded-md">
                    <div class="px-3 py-2 border-b text-sm font-medium text-gray-700 flex items-center justify-between">
                        ...
                        <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs bg-green-100 text-green-700">{{ $countWeek ?? 0 }}</span>
                    </div>
                    <div class="p-3 space-y-2 max-h-96 overflow-y-auto">
                        @forelse(($schedWeek ?? []) as $it)
                            @php $due = $it['due']; @endphp
                            <div class="border rounded-md p-2 bg-green-50 shadow-sm border-l-4 border-green-400">
                                <div class="flex justify-between">
                                    <div class="text-sm font-semibold text-gray-900">{{ $due->format('Y-m-d') }}</div>
                ...
                <div class="bg-white border rounded-md">
                    <div class="px-3 py-2 border-b text-sm font-medium text-gray-700 flex items-center justify-between">
                        ...
                        <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs bg-blue-100 text-blue-700">{{ $countMonth ?? 0 }}</span>
                    </div>
                    <div class="p-3 space-y-2 max-h-96 overflow-y-auto">
                        @forelse(($schedMonth ?? []) as $it)
                            @php $due = $it['due']; @endphp
                            <div class="border rounded-md p-2 bg-blue-50 shadow-sm border-l-4 border-blue-400">
                                <div class="flex justify-between">
                                    <div class="text-sm font-semibold text-gray-900">{{ $due->format('Y-m-d') }}</div>
